    ...
    public void onStart(Intent intent, int startId) {
        ...
        if (!TextUtils.isEmpty(downUrl)) {
            if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
                ...
                final String fileName = filePath + "/" + getPackageName(this) + "-v" + getVersionName(this) + ".apk";
    ...
    class DownFileAsyncTask extends AsyncTask<String, Integer, File> {
        ...
        protected File doInBackground(String... params) {
            try {
                ...
                intent.putExtra(RECEIVER_TYPE_TAG, RECEIVER_TAG_ERROR);
                intent.putExtra(RECEIVER_TAG_ERROR, e.toString());
        ...
        protected void onProgressUpdate(Integer... values) {
            ...
            int rate = values[0];
            if (rate != lastPosition) {
                lastPosition = rate;
                if (showType == UpdateConfig.TYPE_NITIFICATION || showType
                        == UpdateConfig.TYPE_DIALOG_WITH_BACK_DOWN) {
                    if (rate < RECEIVER_TAG_DOING_PROGRESS_MAX) {
                        //更新进度
                        RemoteViews contentView = mNotification.contentView;
                        contentView.setTextViewText(R.id.rate, rate + "%");
                        contentView.setProgressBar(R.id.progress, RECEIVER_TAG_DOING_PROGRESS_MAX, rate, false);
                        // 最后别忘了通知一下,否则不会更新
                        mNotificationManager.notify(NOTIFY_ID, mNotification);
                    } else {
                        //下载完毕后变换通知形式
                ...
                Intent intent = new Intent();
                intent.setAction("android.intent.action.MY_RECEIVER");
                intent.putExtra(RECEIVER_TYPE_TAG, RECEIVER_TAG_DOING);
                intent.putExtra(RECEIVER_TAG_DOING_PROGRESS, rate);
                LocalBroadcastManager.getInstance(getApplicationContext()).sendBroadcast(intent);
            }
        }
    }
