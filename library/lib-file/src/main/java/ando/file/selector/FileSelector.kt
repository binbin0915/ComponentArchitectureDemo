...
class FileSelector private constructor(builder: Builder) {
    ...
    private var mMinCount: Int =
        0                              //可选文件最小数量(Minimum number of optional files)
    private var mMaxCount: Int =
        Int.MAX_VALUE                  //可选文件最大数量(Maximum number of optional files)
    ...
    private var mAllFilesMaxSize: Long =
        -1                     //总文件大小控制(Total file size control) byte (B)
    ...
    fun choose(context: Any, mimeType: String?): FileSelector {
        ...
        if (mStartForResult == null) {
            startActivityForResult(
                context, createChooseIntent(mimeType, mExtraMimeTypes, mIsMultiSelect), mRequestCode
            )
            return this
        }
        when (context) {
            is ComponentActivity -> {// androidx.activity.ComponentActivity
                mStartForResult?.launch(
                    createChooseIntent(
                        mimeType, mExtraMimeTypes, mIsMultiSelect
                    )
                )
            }
            is Fragment -> {
                ...
                startActivityForResult(
                    context,
                    createChooseIntent(mimeType, mExtraMimeTypes, mIsMultiSelect),
                    mRequestCode
                )
            }
        }
    ...
    private fun handleMultiSelectCase(intent: Intent?): FileSelector {
        ...
        (0 until itemCount).forEach { i ->
            ...
            filterUri(uri) { o: FileSelectOptions?, t: IFileType, tf: Boolean, s: Long, sf: Boolean ->
                ...
                if (relationMap[realOption] == null) relationMap[realOption] =
                    SelectResult(checkPass = true)
                val selectResult: SelectResult =
                    relationMap[realOption] ?: SelectResult(checkPass = true)
                ...
                if (!sf) {
                    ...
                    if (isStrictStrategy) {
                        mFileSelectCallBack?.onError(
                            Throwable(
                                realOption.singleFileMaxSizeTip ?: mSingleFileMaxSizeTip
                            )
                        )
                        isNeedBreak = true
                        return@filterUri
            ...
            when (mOverLimitStrategy) {
                OVER_LIMIT_EXCEPT_ALL -> {
                    ...
                    relationMap.filter { (_: FileSelectOptions, v: SelectResult) ->
                        ...
                        if (resultList.isNotEmpty()) resultList.clear()
                        !v.checkPass
                    }.keys.toMutableList().map { o: FileSelectOptions? -> o?.fileType }
        ...
        if (!isFileCountIllegal && !isFileTypeIllegal && !relationMap.isNullOrEmpty()) mFileSelectCallBack?.onSuccess(
            createResult(resultList)
        )
        return this
    }
        ...
        mFileTypeComposite.apply {
            ...
            forEach {
                if (it.getMimeTypeArray().isNullOrEmpty()) {
                    if (it.getMimeType()?.equals(fileType.getMimeType(), true) == true) fileType =
                        it
                } else {
                    if (it.getMimeTypeArray()
                            ?.contains(UNKNOWN.parseSuffix(uri)) == true
                    ) fileType = it
                }
    ...
    private fun filterUri(
        ...
        val currentOption: List<FileSelectOptions>? =
            mFileSelectOptions?.filter { it.fileType == fileType }
        ...
        if (currentOption.isNullOrEmpty()) {
            ...
            block.invoke(
                null, fileType, false, fileSize, limitFileSize(fileSize, realSizeLimit(null))
            )
            return
        }

        if (isOptionsNullOrEmpty) {
            val isAccept = (mFileSelectCondition != null) && (mFileSelectCondition?.accept(
                fileType, uri
            ) == true)
            if (!isAccept) return
            block.invoke(
                null, fileType, true, fileSize, limitFileSize(fileSize, realSizeLimit(null))
            )
        } else {
            currentOption.forEach { o: FileSelectOptions ->
                ...
                val isAccept =
                    mFileSelectCondition?.accept(fileType, uri) ?: true && o.fileCondition?.accept(
                        fileType, uri
                    ) ?: true
                if (!isAccept) {
                    block.invoke(
                        o, fileType, true, fileSize, limitFileSize(fileSize, realSizeLimit(o))
                    )
                    return@forEach
                }
    ...
    private fun realMinCountTip(option: FileSelectOptions?): String =
        option?.minCountTip ?: mMinCountTip

    private fun realMaxCountTip(option: FileSelectOptions?): String =
        option?.maxCountTip ?: mMaxCountTip
    ...
    private fun realMaxCountLimit(option: FileSelectOptions?): Int {
        return max(
            ...
            if (option?.maxCount ?: Int.MAX_VALUE > 0) min(
                option?.maxCount ?: Int.MAX_VALUE,
                mRealMaxCount
            )
            else mRealMaxCount
        )
    ...
    private fun createResult(
        ...
        fileType: IFileType,
        fileSize: Long,
    ): MutableList<FileSelectResult> = mutableListOf<FileSelectResult>().apply {
    ...
    class Builder internal constructor(
        private val context: Any, launcher: ActivityResultLauncher<Intent>?
    ) {
        var mStartForResult: ActivityResultLauncher<Intent>? = launcher
        var mRequestCode: Int = 0

        var mExtraMimeTypes: Array<out String>? =
            null      //eg: Intent.putExtra(Intent.EXTRA_MIME_TYPES, mExtraMimeTypes)
        var mIsMultiSelect: Boolean = false
        var mMinCount: Int =
            0                              //可选文件最小数量(Minimum number of optional files)
        var mMaxCount: Int =
            0                              //可选文件最大数量(Maximum number of optional files)
        var mMinCountTip: String = TIP_COUNT_MIN
        var mMaxCountTip: String = TIP_COUNT_MAX

        var mSingleFileMaxSize: Long =
            -1                   //单文件大小控制 byte (B) (Single file size control)
        var mAllFilesMaxSize: Long =
            -1                     //总文件大小控制 byte (B) (Total file size control)
        ...
        fun setExtraMimeTypes(vararg extraMimeTypes: String): Builder {
            this.mExtraMimeTypes =
                if (extraMimeTypes.size == 1) arrayOf(extraMimeTypes[0]) else extraMimeTypes
            return this
        }
